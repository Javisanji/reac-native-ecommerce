...
export default function AddAddress(props) {
	...
	useEffect(() => {
		(async () => {
			if (params?.idAddress) {
				setNewAddress(false);
				navigation.setOptions({ title: "Atualização de Endereço" });
	...
	return (
		<KeyboardAwareScrollView extraScrollHeight={25}>
			<View style={styles.container}>
				<Text style={styles.title}>Nova direção</Text>
				<TextInput
					label="Título"
				...
				<Button
					...
					loading={loading}
				>
					{newAddress ? "Criar endereço" : "Atualização de Endereço"}
				</Button>
			</View>
